...
describe ToyRobot::Functional do
  ...
  describe '#initialize' do
    context 'when put inside table limits' do
      let(:toy_robot) { ToyRobot::Functional.new(table, 0, 0, 'NORTH') }
    ...
    context 'when put outside tabletop limits' do
      it 'should raise error' do
        expect{ ToyRobot::Functional.new(table, 5, 0, 'WEST') }.to raise_error(ToyRobot::InvalidPosition)
      end
    end

    context 'using wrong facing value' do
      it 'should raise error' do
        expect{ ToyRobot::Functional.new(table, 0, 0, 'SOUTH WEST') }.to raise_error(ToyRobot::InvalidFacingValue)
      end
    end
  end
  ...
  describe '#report' do
    it 'should return the current position and facing direction of toy robot' do
      toy_robot = ToyRobot::Functional.new(table, 0, 0, 'NORTH')
  ...
  describe '#left' do
    ...
    context 'with robot facing EAST' do
      it 'should set robot to face NORTH' do
        toy_robot = ToyRobot::Functional.new(table, 0, 0, 'EAST')
        toy_robot.left
        expect(toy_robot.facing).to eq('NORTH')
      end
    end
    ...
    context 'with robot facing SOUTH' do
      it 'should set robot to face EAST' do
        toy_robot = ToyRobot::Functional.new(table, 0, 0, 'SOUTH')
        toy_robot.left
    ...
    context 'with robot facing WEST' do
      it 'should set robot to face SOUTH' do
        toy_robot = ToyRobot::Functional.new(table, 0, 0, 'WEST')
        toy_robot.left
  ...
  describe '#move' do
    describe 'move the toy robot forward the direction it is facing' do
      context 'on valid tabletop positions' do
        ...
        context 'with robot facing SOUTH' do
          it 'should move robot -1 unit on Y axis' do
            toy_robot = ToyRobot::Functional.new(table, 0, 1, 'SOUTH')
            toy_robot.move
        ...
        context 'with robot facing WEST' do
          it 'should move robot 1 unit on X axis' do
            toy_robot = ToyRobot::Functional.new(table, 4, 0, 'WEST')
            toy_robot.move
      ...
      context 'with invalid movement' do
        let(:toy_robot) { ToyRobot::Functional.new(table, 0, 0, 'WEST') }
